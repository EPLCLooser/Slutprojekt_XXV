...
def get_event(event_id)
  ...
  if 
    event = db.execute("SELECT event_types.name AS event_type_name, * FROM ((event_types INNER JOIN events ON events.event_type_id = event_types.id) INNER JOIN users_events ON events.id = users_events.event_id) WHERE events.id=? AND user_id=?", [event_id, session[:user_id]]).first
  else
    session[:error] = "You are an user in this event"
